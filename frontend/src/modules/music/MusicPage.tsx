...
export default function MusicPage() {
  ...
  return (
    <div className="page">
      ...
      <p className="hint">全屏黑胶听歌界面：唱臂落盘、封面糊化为背景、逐句滚动歌词。「一起听」开启后，同一账号的其它设备会通过云端房间实时跟随播放进度（两端需各自存有该曲目）。</p>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <div
              ...
              style={{
                ...
                background: 'radial-gradient(circle at 50% 50%, var(--soft) 38%, rgba(0,0,0,0.25) 39%, var(--soft) 41%)',
              }}
            >
              <div style={{ width: 14, height: 14, borderRadius: '50%', background: 'var(--acc)' }} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 600, fontSize: '1.05rem' }}>{track.name}</div>
              <input
                type="range"
                min={0}
                max={dur || 0}
      ...
      {track && track.lrc.length > 0 && (
        <GlassCard>
          <div style={{ maxHeight: 200, overflowY: 'auto', textAlign: 'center', lineHeight: 2.2 }}>
            {track.lrc.map((l, i) => (
              <div key={i} style={{ color: i === activeLyric ? 'var(--acc)' : 'var(--tx3)', fontWeight: i === activeLyric ? 600 : 400, transition: 'color .2s' }}>
                {l.text}
              </div>
            ))}
          </div>
      ...
      <GlassCard>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <input className="fld" value={deviceLabel} onChange={(e) => { setDeviceLabel(e.target.value); kvSet('deviceLabel', e.target.value); }} placeholder="本机设备名" style={{ flex: 1 }} />
        </div>
